...
import { useDeletePurchaseInvoice } from "@/hooks/useDeleteInvoice";
import { usePermissions } from "@/hooks/usePermissions";

type SortField = "date" | "number" | "supplier" | "subtotal" | "total";
type SortDir = "asc" | "desc";

const ConfirmedPurchaseInvoices = () => {
  ...
  const { data: invoices, isLoading } = useQuery({
    queryKey: ["confirmed_purchase_invoices"],
    queryFn: async () => {
  ...
  const processed = useMemo(() => {
    ...
    result = [...result].sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case "supplier": cmp = ((a.suppliers as any)?.name || "").localeCompare((b.suppliers as any)?.name || ""); break;
      ...
      <div className="space-y-6 animate-fade-in">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            ...
            فواتير الشراء المؤكدة
          </h1>
          <p className="text-muted-foreground">عرض جميع فواتير الشراء التي تم تأكيدها</p>
        </div>
                  ...
                  processed.map((inv) => (
                    <TableRow key={inv.id}>
                      ...
                      <TableCell>{(inv.suppliers as any)?.name}</TableCell>
                       ...
                       <TableCell>
                         {can("invoices_confirmed_purchases", "delete") && (
                           <AlertDialog>
...
export default ConfirmedPurchaseInvoices;
